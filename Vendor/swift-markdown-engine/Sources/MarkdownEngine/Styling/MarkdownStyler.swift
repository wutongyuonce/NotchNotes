...
extension MarkdownStyler {
    ...
    static func styleStrikethrough(_ ctx: StylingContext) -> [StyledRange] {
        var attrs: [StyledRange] = []
        for (idx, token) in ctx.tokens.enumerated() where token.kind == .strikethrough {
            ...
            if !isActive {
                let smallFont = NSFont(name: ctx.fontName, size: ctx.configuration.markers.hiddenMarkerFontSize)
                    ?? NSFont.systemFont(ofSize: ctx.configuration.markers.hiddenMarkerFontSize)
                token.markerRanges.forEach { range in
                    attrs.append((range, [
                        .font: smallFont,
    ...
    static func styleBlockquotes(_ ctx: StylingContext) -> [StyledRange] {
        ...
        for token in ctx.tokens where token.kind == .blockquote {
            ...
            let quoteColor = ctx.configuration.theme.mutedText.withAlphaComponent(0.6)
            attrs.append((paraRange, [
                .paragraphStyle: para,
                .foregroundColor: quoteColor
            ]))
            token.markerRanges.forEach { range in
                attrs.append((range, [.foregroundColor: NSColor.clear]))
            }
        }
        return attrs
    }
